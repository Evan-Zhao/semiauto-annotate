...
# from labelme_server import ModelLoader
from .. import conf
from ..conf import PREPROCESSED_YOLO_RESULT_PATH, PREPROCESSED_POSE_ESTM_RESULT_PATH

mongodb_client = pymongo.MongoClient(conf.MONGODB_URL)
mongodb_db = mongodb_client[conf.MONGODB_DATABASE]
mongodb_collection = mongodb_db[conf.MONGODB_COLLECTION]


def get_image_list():
    result = mongodb_collection.find({}, {'filename': 1})
...
def modify_collection_row(img_id, column, content):
    ...
    mongodb_collection.update({'id': img_id}, result)


def get_unpreprocessed_img():
    results = mongodb_collection.find({"preprocessed": False})
    return results
...
def get_incomplete_img():
    ...
    result = mongodb_collection.find_one({"$and":
                                              [{'complete': False},
                                               {'in_use': False},
                                               {'preprocessed': True}]})
